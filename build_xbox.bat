...
if "%XBOX_PERF_SMOKE%"=="1" (
    echo  Xbox perf smoke instrumentation: ENABLED
    set CFLAGS=%CFLAGS% /DXBOX_PERF_SMOKE=1
)
if "%XBOX_VERBOSE_DEBUG%"=="1" (
    echo  Xbox verbose formatted debug logging: ENABLED
    set CFLAGS=%CFLAGS% /DXBOX_VERBOSE_DEBUG=1
)
...
for %%F in (
    ...
    src\Platform\Xbox\xbox_systemlink_probe.c
...
>> "!RSPFILE!" echo d3d8.lib d3dx8.lib dsound.lib xboxkrnl.lib xgraphics.lib xnet.lib xonline.lib xmv.lib libc.lib xapilib.lib
