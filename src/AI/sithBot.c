...
/* Match roster survives deaths, resets with the world. */
static int sithBot_rosterReady;
static int sithBot_rosterModel[JKPLAYER_NUM_INFOS];
static int sithBot_rosterEasy[JKPLAYER_NUM_INFOS];

static void sithBot_PrepareRoster(void)
{
    int i;
    int firstEasy;
    if (sithBot_rosterReady) return;
    firstEasy = _frand() < 0.5;
    for (i = 0; i < JKPLAYER_NUM_INFOS; ++i)
    {
        sithBot_rosterModel[i] = i % 8;
        sithBot_rosterEasy[i] = (i + firstEasy) & 1;
    }
    for (i = 7; i > 0; --i)
    {
        int j = (int)(_frand() * (i + 1));
        int tmp;
        if (j > i) j = i;
        tmp = sithBot_rosterModel[i];
        sithBot_rosterModel[i] = sithBot_rosterModel[j];
        sithBot_rosterModel[j] = tmp;
    }
    for (i = 8; i < JKPLAYER_NUM_INFOS; ++i)
        sithBot_rosterModel[i] = sithBot_rosterModel[i % 8];
    sithBot_rosterReady = 1;
}
...
/* Inclusive microseconds, enabled only by -bot-profile. */
static uint64_t sithBot_profileUs[4];
static uint64_t sithBot_ProfileTimeUs(void)
{
#if defined(TARGET_XBOX) || defined(PLATFORM_POSIX)
    return Linux_TimeUs();
#else
    return (uint64_t)stdPlatform_GetTimeMsec() * 1000;
#endif
}
...
#ifdef TARGET_XBOX
#if defined(XBOX_VERBOSE_DEBUG)
    xbox_debug_PerfPrintf("%s", buf);
#else
    /* Keep navigation/startup diagnostics on disk. Per-action traces must
       not force a write-through disk flush in the simulation loop; the
       native RAM monitor still receives every message. */
    if (!strncmp(buf, "BotMatch:", 9) || !strncmp(buf, "BotPerf:", 8) ||
        !strncmp(buf, "BotProfile:", 11) || !strncmp(buf, "BotLiftProbe:", 13))
        xbox_debug_Trace(buf);
    else
...
static int sithBot_CanSeePositionImpl(sithSector *fromSector, const rdVector3 *fromPos, sithSector *toSector, const rdVector3 *toPos)
{
    rdVector3 end;
...
static int sithBot_CanSeePosition(sithSector *fromSector, const rdVector3 *fromPos, sithSector *toSector, const rdVector3 *toPos)
{
    ...
    int result;
    if (!Main_botProfile)
        return sithBot_CanSeePositionImpl(fromSector, fromPos, toSector, toPos);
    startUs = sithBot_ProfileTimeUs();
    result = sithBot_CanSeePositionImpl(fromSector, fromPos, toSector, toPos);
    sithBot_profileUs[0] += sithBot_ProfileTimeUs() - startUs;
    return result;
}
...
/* Scoped to one read-only nearest-node query. Never retained across bot
   actions, physics updates, or door/lift/sector changes. */
typedef struct SithBotFloorQuery
{
    sithThing *probe;
    sithSector *sector;
    rdVector3 pos;
...
typedef struct SithBotFloorQueryCache
{
    int count;
    SithBotFloorQuery entries[16];
} SithBotFloorQueryCache;
static SithBotFloorQueryCache *sithBot_floorQueryCache;
static uint32_t sithBot_floorQueryHits;

static int sithBot_CachedFloorQuery(sithThing *probe, sithSector *sector,
                                   const rdVector3 *pos, flex_t rise)
{
    int i;
    if (!sithBot_floorQueryCache || !pos)
        return -1;
    for (i = 0; i < sithBot_floorQueryCache->count; ++i)
    {
        SithBotFloorQuery *entry = &sithBot_floorQueryCache->entries[i];
        if (entry->probe == probe && entry->sector == sector && entry->rise == rise &&
            entry->pos.x == pos->x && entry->pos.y == pos->y && entry->pos.z == pos->z)
        {
            ++sithBot_floorQueryHits;
            return entry->result;
        }
    }
    return -1;
...
static void sithBot_StoreFloorQuery(sithThing *probe, sithSector *sector,
                                   const rdVector3 *pos, flex_t rise, int result)
{
    SithBotFloorQuery *entry;
    if (!sithBot_floorQueryCache || !pos || sithBot_floorQueryCache->count >= 16)
        return;
    entry = &sithBot_floorQueryCache->entries[sithBot_floorQueryCache->count++];
    entry->probe = probe;
    entry->sector = sector;
    entry->pos = *pos;
    entry->rise = rise;
...
static int sithBot_PositionHasWalkableFloorWithRiseImpl(sithThing *probeThing, sithSector *sector, const rdVector3 *pos, flex_t stepHeight)
{
    rdVector3 down;
...
{
    uint64_t startUs = 0;
    int result = sithBot_CachedFloorQuery(probeThing, sector, pos, stepHeight);
    if (result >= 0)
        return result;
    if (Main_botProfile)
        startUs = sithBot_ProfileTimeUs();
    result = sithBot_PositionHasWalkableFloorWithRiseImpl(probeThing, sector, pos, stepHeight);
    if (Main_botProfile)
        sithBot_profileUs[1] += sithBot_ProfileTimeUs() - startUs;
    sithBot_StoreFloorQuery(probeThing, sector, pos, stepHeight, result);
    return result;
}
...
static int sithBot_IsWalkableSegment(const SithBotNode *from, const SithBotNode *to)
{
    ...
    for (i = 1; i <= samples; i++)
    {
        ...
        if (sithBot_IsDynamicHazardSector(sector) && sector != from->sector)
            return 0;
        /* The footprint check below includes this exact center-floor query.
         * Only query it separately when no footprint will be checked. */
        if ((assistedVertical || !probeThing) &&
            !(assistedVertical && sithBot_IsUpwardThrustSector(sector)) &&
...
static void sithBot_ResolvePortalRouteFloors(void)
{
    int i;
    int resolved = 0;
    for (i = 0; i < sithBot_numNodes; ++i)
    {
        ...
        node->routeFloorZ = 0.0;
        if (node->kind != SITHBOT_NODE_PORTAL)
            continue;

        /* Portal min-Z describes the opening, not necessarily the floor on
         * either approach. Resolve static support once per graph load, so
         * route acceptance does not repeatedly trace or wait above a ramp. */
        rdVector_Copy3(&start, &node->pos);
        start.z += 0.10;
        sector = sithCollision_GetSectorLookAt(node->sector, &node->pos, &start, 0.01);
        if (!sithBot_IsNavSectorUsableForBot(sector))
            continue;
        rdVector_Neg3(&down, &rdroid_zVector3);
        sithCollision_SearchRadiusForThings(sector, 0, &start, &down,
            0.50, 0.0, RAYCAST_1 | RAYCAST_2000 | RAYCAST_2);
        while ((entry = sithCollision_NextSearchResult()) != 0)
        {
            if (!(entry->hitType & SITHCOLLISION_WORLD))
                continue;
            if (sithBot_IsSurfaceWalkableForBot(entry->surface))
            {
                node->routeFloorZ = start.z - entry->distance;
                node->routeFloorKnown = 1;
                ++resolved;
            }
            /* Never look through a nonwalkable floor to accept support below. */
            break;
        }
        sithCollision_SearchClose();
    }
    sithBot_Logf("BotNav: portal-floor-support resolved=%d\n", resolved);
}

static flex_t sithBot_GetRouteStandingRise(sithThing *thing, const SithBotNode *node)
{
    flex_t rise = node->pos.z - thing->position.z;
    if (node->kind == SITHBOT_NODE_PORTAL && node->routeFloorKnown)
        return node->routeFloorZ + sithPhysics_ThingGetInsertOffsetZ(thing) - thing->position.z;
    if (node->kind == SITHBOT_NODE_FLOOR || node->kind == SITHBOT_NODE_PORTAL)
        rise += sithPhysics_ThingGetInsertOffsetZ(thing) - 0.04;
    return rise;
}

static int sithBot_IsBelowRouteFloor(sithThing *thing, const SithBotNode *node)
{
    if (node->kind != SITHBOT_NODE_FLOOR && node->kind != SITHBOT_NODE_PORTAL)
        return 0;
    /* These graph samples are surface+0.04, not standing actor origins. */
    return sithBot_GetRouteStandingRise(thing, node) > 0.04;
}
...
static int sithBot_IsRouteNodeReached(sithThing *thing, const SithBotNode *node, flex_t normalRadius)
{
    ...
    if (!thing || !node || sithBot_IsBelowRouteFloor(thing, node))
        return 0;
...
static int sithBot_FindNearestNodeAtImpl(sithSector *sector, const rdVector3 *pos)
{
    ...
    for (pass = 0; pass < 2; pass++)
    {
        ...
        for (i = 0; i < candidateCount; i++)
        {
            ...
            /* IsWalkableSegment includes the same visibility trace. */
...
{
    SithBotFloorQueryCache cache;
    SithBotFloorQueryCache *previousCache = sithBot_floorQueryCache;
    uint64_t startUs = 0;
    int result;
    cache.count = 0;
    ...
    if (Main_botProfile)
        startUs = sithBot_ProfileTimeUs();
    result = sithBot_FindNearestNodeAtImpl(sector, pos);
    if (Main_botProfile)
        sithBot_profileUs[2] += sithBot_ProfileTimeUs() - startUs;
    sithBot_floorQueryCache = previousCache;
    return result;
}
...
/* A search sees one simulation timestamp and an unchanged graph. Snapshot
 * owner/shared blocks once, rather than scanning all 128 records per edge.
 * The mask is local to the search, so expiry and subsequent blocks are never
 * hidden by a persistent cache. MAX_EDGES is 24, fitting one uint32_t. */
static void sithBot_BuildRouteBlockedMasks(int ownerSlot, uint32_t *masks)
{
    int i;
    memset(masks, 0, sithBot_numNodes * sizeof(*masks));
    if (ownerSlot < 0)
        return;
    for (i = 0; i < SITHBOT_MAX_BLOCKED_EDGES; ++i)
    {
        ...
        if (blocked->untilMs <= sithTime_curMs ||
            (blocked->ownerSlot != ownerSlot &&
             blocked->ownerSlot != SITHBOT_BLOCK_SHARED_OWNER) ||
            blocked->fromNode < 0 || blocked->fromNode >= sithBot_numNodes)
            continue;
        from = &sithBot_nodes[blocked->fromNode];
        for (edge = 0; edge < from->edgeCount; ++edge)
            if (from->edges[edge] == blocked->toNode)
                masks[blocked->fromNode] |= (uint32_t)1 << edge;
    }
}
...
static int sithBot_FindPathNextWeightedImpl(int ownerSlot, int startNode, int goalNode,
                                        sithSector *avoidSector)
{
    uint32_t blockedMasks[SITHBOT_MAX_NODES];
    ...
    sithBot_BuildRouteBlockedMasks(ownerSlot, blockedMasks);
    for (i = 0; i < sithBot_numNodes; i++)
    {
        prev[i] = -2;
        heapPos[i] = -1;
    ...
    while (heapCount > 0)
    {
        ...
        for (i = 0; i < sithBot_nodes[node].edgeCount; i++)
        {
            ...
            if (closed[next] || (blockedMasks[node] & ((uint32_t)1 << i)))
                continue;
                                        ...
                                        sithSector *avoidSector)
{
    ...
    int result;
    if (!Main_botProfile)
        return sithBot_FindPathNextWeightedImpl(ownerSlot, startNode, goalNode, avoidSector);
    startUs = sithBot_ProfileTimeUs();
    result = sithBot_FindPathNextWeightedImpl(ownerSlot, startNode, goalNode, avoidSector);
    sithBot_profileUs[3] += sithBot_ProfileTimeUs() - startUs;
    return result;
}
...
static int sithBot_FindRouteMoveNode(int startNode, int goalNode, sithThing *thing)
{
    ...
    while (thing && nextNode >= 0 && nextNode != goalNode && skipped < 8)
    {
        ...
        if (sithBot_IsBelowRouteFloor(thing, &sithBot_nodes[nextNode]) ||
            (!sameSectorReached && !overlappingTransition && !clearanceReached))
            break;
        startNode = nextNode;
...
static int sithBot_FindCommittedRouteMoveNode(SithBotState *state, int startNode, int goalNode, sithThing *thing)
{
    ...
    if (state->routeGoalNode == goalNode && state->nextNode >= 0 && state->nextNode < sithBot_numNodes)
    {
        ...
        if (sithBot_IsBelowRouteFloor(thing, &sithBot_nodes[state->nextNode]))
...
static void sithBot_DriveGroundVelocity(SithBotState *state, sithThing *thing, const rdVector3 *flatDir, flex_t dist, flex_t targetDz, int combat)
{
    ...
    /* Keep the authored player speed: bot difficulty must not raise it. */
    if (desiredSpeed <= 0.0)
        desiredSpeed = 1.0;
    if (desiredSpeed > 4.2)
        desiredSpeed = 4.2;
...
static void sithBot_ApplyCharacterModel(int playerIdx, sithThing *thing)
{
    ...
    sithBot_PrepareRoster();
    modelIdx = sithBot_rosterModel[playerIdx % JKPLAYER_NUM_INFOS];
...
static void sithBot_Respawn(int botIdx)
{
    ...
    /* Start the explicit lift fixture at an authored lower-landing spawn.
     * The normal navigability selection and spawn pipeline still apply. */
    if (Main_bAutostart && Main_botLiftProbe == 1 && botIdx == 1 &&
        !__strcmpi(Main_strEpisode, "JK1MP") && !__strcmpi(Main_strMap, "m2.jkl") &&
        jkPlayer_maxPlayers > 4 && jkPlayer_playerInfos[4].pSpawnSector)
        preferredSpawnIdx = 4;
...
static void sithBot_ResetState(SithBotState *bot, int playerIdx)
{
    ...
    sithBot_PrepareRoster();
    bot->easy = sithBot_rosterEasy[playerIdx];
    bot->active = 1;
...
static int sithBot_ChooseTeam(int playerIdx)
{
    int counts[3] = {0, 0, 0};
    int i;
    if (!sithBot_IsTeamMode()) return 0;
    for (i = 0; i < jkPlayer_maxPlayers; ++i) {
        sithPlayerInfo *info = &jkPlayer_playerInfos[i];
        if (i != playerIdx && (info->flags & 1) &&
            info->teamNum >= 1 && info->teamNum <= 2)
            ++counts[info->teamNum];
    }
    if (counts[1] == counts[2]) return _frand() < 0.5 ? 1 : 2;
...
static void sithBot_ActivateSlot(int playerIdx, int botNumber)
{
    ...
    info->score = 0;
    info->respawnMask = 0;
    selectedTeam = sithBot_ChooseTeam(playerIdx);
    info->teamNum = selectedTeam;
    ...
    if (sithBot_IsTeamMode())
        /* CTF's join callback clears teamNum while placing players in staging. */
        sithBot_EnterCtfTeam(thing, selectedTeam);
    if (sithComm_multiplayerFlags)
        sithDSSThing_SendSyncThing(thing, -1, 255);
    sithMulti_SyncScores();
    sithBot_NewBotState(playerIdx);

    sithBot_Logf("BotMatch: joined slot=%d net=%u team=%d name='%S' skill=%s\n",
                 playerIdx,
                 info->net_id,
                 info->teamNum,
                 info->player_name,
                 sithBot_rosterEasy[playerIdx] ? "easy" : "medium");
}
...
static void sithBot_UpdateCamera(void)
{
    ...
    if (sithBot_cameraPlayer != targetIdx || sithWorld_pCurrentWorld->cameraFocus != target ||
        sithCamera_cameras[cameraIdx].primaryFocus != target)
    {
        sithWorld_pCurrentWorld->cameraFocus = target;
        sithCamera_SetsFocus();
        sithBot_cameraPlayer = targetIdx;
        sithBot_Logf("BotMatch: bot-camera slot=%d mode=%s\n", targetIdx,
                     cameraIdx == 0 ? "first-person" : "third-person");
    }

    if (Main_bAutostart && Main_botLiftProbe) {
        /* Probe-only framing: show the passenger's feet and platform contact. */
        sithCamera_cameras[cameraIdx].collisionOffset.y = -0.4;
        sithCamera_cameras[cameraIdx].collisionOffset.z = 0.24;
    }

    if (sithCamera_currentCamera != &sithCamera_cameras[cameraIdx])
        sithCamera_SetCurrentCamera(&sithCamera_cameras[cameraIdx]);
}
...
static int sithBot_HandleControlledHazardRoute(SithBotState *state, sithThing *bot, int nextNode)
{
    ...
    /* An empty authored trap set cannot intercept any route. Avoid the
     * sector-walking traces entirely in ordinary deathmatch arenas. */
    if (!sithBot_numControlledHazards)
        return 0;
...
static int sithBot_TryAttachToPathLift(sithThing *thing, sithThing *lift)
{
    ...
    rdVector3 probeStart;
    rdVector3 worldNormal;
    rdVector3 surfacePoint;
    rdVector3 standingPos;
    flex_t correction;
    sithSector *standingSector;
    int attached = 0;
    ...
    while ((entry = sithCollision_NextSearchResult()) != 0)
    {
        ...
        /* LandThing records the existing relative position; NO_MOVE prevents
         * the normal physics pass from raising an embedded origin afterwards.
         * Resolve the actual face plane and standing height before latching. */
        rdMatrix_TransformVector34(&surfacePoint,
            &entry->sender->vertices[entry->face->vertexPosIdx[0]], &lift->lookOrientation);
        rdVector_Add3Acc(&surfacePoint, &lift->position);
        rdVector_Sub3(&standingPos, &thing->position, &surfacePoint);
        correction = (sithPhysics_ThingGetInsertOffsetZ(thing) -
            rdVector_Dot3(&standingPos, &worldNormal)) / worldNormal.z;
        if (correction < -0.12 || correction > 0.35)
            continue;
        rdVector_Copy3(&standingPos, &thing->position);
        standingPos.z += correction;
        standingSector = sithCollision_GetSectorLookAt(thing->sector,
            &thing->position, &standingPos, thing->moveSize);
        if (!sithBot_IsNavSectorUsableForBot(standingSector))
            continue;
        /* GetSectorLookAt may shorten the move against a ceiling. Do not
         * record a standing attachment if that prevents the required height. */
        if (fabs(standingPos.z - (thing->position.z + correction)) > 0.001)
            continue;
        if (Main_bAutostart && Main_botLiftProbe)
            sithBot_Logf("BotLiftProbe: contact bot=%d lift=%d surfaceZ=%.4f standingZ=%.4f clearance=%.4f correction=%.4f\n",
                thing->thingIdx, lift->thingIdx, surfacePoint.z, standingPos.z,
                sithPhysics_ThingGetInsertOffsetZ(thing), correction);
        rdVector_Copy3(&thing->position, &standingPos);
        if (thing->sector != standingSector)
            sithThing_MoveToSector(thing, standingSector, 0);
                                               ...
                                               sithThing *lift)
{
    ...
    if (Main_bAutostart && Main_botLiftProbe) {
        static uint32_t nextContactLog;
        if (sithTime_curMs >= nextContactLog) {
            sithBot_Logf("BotLiftProbe: ride ms=%u bot=%d lift=%d z=%.4f liftZ=%.4f relativeZ=%.4f attach=0x%X frame=%d goal=%d moving=%d\n",
                sithTime_curMs, passenger->thingIdx, lift->thingIdx,
                passenger->position.z, lift->position.z, passenger->field_4C.z,
                passenger->attach_flags, lift->curframe, lift->goalframe,
                (lift->trackParams.flags & 3) != 0);
            nextContactLog = sithTime_curMs + 250;
        }
    }
...
static int sithBot_HandlePathLiftRoute(SithBotState *state, sithThing *thing, int nextNode)
{
    ...
    if (!attached && state->ridingLiftThingIdx == lift->thingIdx)
    {
        ...
        {
            /* Reacquire a real standing contact; proximity to a moving car
             * alone must never pin a passenger inside its mesh. */
            attached = sithBot_TryAttachToPathLift(thing, lift);
            if (!attached) {
...
static void sithBot_RelinkLiftExitSector(sithThing *thing, sithSector *landingSector)
{
    /* A passenger can retain the shaft sector after a mover carries its
       origin through the upper opening. Repair only a proven stale sector;
       the landing must already contain the unchanged actor position. */
    if (!thing || !thing->sector || !landingSector || thing->sector == landingSector ||
        sithIntersect_IsSphereInSector(&thing->position, 0.0, thing->sector) ||
        !sithIntersect_IsSphereInSector(&thing->position, 0.0, landingSector))
        return;
    sithBot_Logf("BotMatch: lift-sector-relink from=%d to=%d pos=(%.3f,%.3f,%.3f)\n",
                 sithBot_GetSectorIndex(thing->sector),
                 sithBot_GetSectorIndex(landingSector),
                 thing->position.x, thing->position.y, thing->position.z);
    sithThing_MoveToSector(thing, landingSector, 0);
}

static int sithBot_HandleLiftExit(SithBotState *state, sithThing *thing)
{
    SithBotNode *exitNode;
    SithBotNode *targetNode;
    sithThing *lift = 0;
    ...
    sithBot_RelinkLiftExitSector(thing, exitTargetSector);
    if (state->ridingLiftThingIdx >= 0)
    ...
    if (sithBot_IsPathLiftThing(lift))
    {
        ...
        /* Graph exit points already describe the walkable landing. Extending
         * them radially beyond the lift can cross a narrow balcony's outer
         * edge (stock m2's upper landing) and send the passenger downstairs. */
            ...
            !sithBot_GetPathLiftForNode(landingNode))
        {
                ...
                sithBot_AbsFlex(sithBot_nodes[landingNode].pos.z - exitNode->pos.z) < 0.30)
            {
                ...
                if (onwardDot > 0.20)
                {
                    ...
                    targetNode = &sithBot_nodes[landingNode];
                    exitTargetSector = targetNode->sector;
                }
    ...
    /* Floor/portal nodes sit 0.04 above the supporting surface. Convert to
     * the actor origin used by FindFloor; comparing the node height directly
     * can stop a jump while the actor's feet are still below the ledge. */
    exitFloorZ = exitTarget.z;
    if (targetNode->kind == SITHBOT_NODE_PORTAL ||
        targetNode->kind == SITHBOT_NODE_FLOOR)
        exitFloorZ += sithPhysics_ThingGetInsertOffsetZ(thing) - 0.04;
    ...
    if ((exitDist < 0.28 ||
        ...
        sithBot_AbsFlex(thing->position.z - exitFloorZ) < 0.25)
    {
        ...
        if (thing->sector == exitTargetSector &&
            (thing->attach_flags & SITH_ATTACH_WORLDSURFACE))
        {
    ...
    if (!thing->attach_flags)
    {
        ...
        if (!thing->attach_flags)
        {
            ...
            if (thing->position.z + 0.02 < exitFloorZ)
            {
    ...
    if (sithBot_IsPathLiftThing(lift))
    {
        ...
        if (thing->position.z + 0.02 < exitFloorZ)
        {
            /* FindFloor above may have attached us to the lower landing.
             * Grounded physics projects velocity onto that floor, so a raw
             * upward velocity is discarded unless a normal jump detaches. */
            if (thing->attach_flags)
                sithPlayerActions_JumpWithVel(thing, 0.45);
            flex_t forwardJumpSpeed = thing->actorParams.jumpSpeed * 0.45;
            if (forwardJumpSpeed < 0.80)
                forwardJumpSpeed = 0.80;
            if (thing->physicsParams.vel.z < forwardJumpSpeed)
...
static void sithBot_MoveToward(SithBotState *state, sithThing *thing, const rdVector3 *target, int combat)
{
    ...
    routeRise = state->nextNode >= 0 && state->nextNode < sithBot_numNodes
        ? sithBot_GetRouteStandingRise(thing, &sithBot_nodes[state->nextNode])
        : 0.0;
    if (!combat &&
        routeRise > 0.10 && routeRise < 1.15 &&
        dist < 1.70 &&
        state->nextNode >= 0 && state->nextNode < sithBot_numNodes &&
        (sithBot_nodes[state->nextNode].kind == SITHBOT_NODE_PORTAL ||
         sithBot_nodes[state->nextNode].kind == SITHBOT_NODE_FLOOR) &&
...
static int sithBot_HandleShortGapTraversal(SithBotState *state, sithThing *thing)
{
    ...
    if (thing->attach_flags && sithBot_IsRouteNodeReached(thing, landing, 0.34))
    {
...
static void sithBot_FireAt(SithBotState *state, sithThing *thing, sithThing *enemy)
{
    ...
    if (sithTime_curMs < state->nextFireMs || sithTime_curMs < state->reactionUntilMs)
        return;
    ...
    if (spec)
    {
        ...
        aimError.x = (_frand() - 0.5) * (spec->spreadDeg + (state->easy ? 5.0 : 2.0));
        aimError.y = (_frand() - 0.5) * (spec->spreadDeg + (state->easy ? 5.0 : 2.0));
        aimError.z = 0.0;
        ...
        state->nextFireMs = sithTime_curMs + (uint32_t)(spec->fireWaitMs * (state->easy ? 1.65 : 1.15) + _frand() * spec->fireWaitMs * 0.45);
        return;
    }
...
/* Explicit autostart-only stock-map probe. Assign goals to the ordinary
 * route controller; never alter position, sectors, attachment or collision. */
static int sithBot_liftProbePhase;
static uint32_t sithBot_liftProbeReadyMs;
static int sithBot_LiftProbeGoal(SithBotState *state, sithThing *thing)
{
    int goal;
    if (!Main_bAutostart || !Main_botLiftProbe || state->playerIdx != 1 ||
        __strcmpi(Main_strEpisode, "JK1MP") || __strcmpi(Main_strMap, "m2.jkl") ||
        sithBot_numNodes <= 144)
        return -1;
    if (!sithBot_liftProbeReadyMs)
        sithBot_liftProbeReadyMs = sithTime_curMs + 8000;
    if (sithTime_curMs < sithBot_liftProbeReadyMs)
        return -2;
    if (sithBot_liftProbePhase >= 3)
        return -2;
    goal = sithBot_liftProbePhase == 1 ? 144 : 4;
    if (thing->sector == sithBot_nodes[goal].sector &&
        sithBot_DistSq(&thing->position, &sithBot_nodes[goal].pos) < 0.04 &&
        (thing->attach_flags & SITH_ATTACH_WORLDSURFACE))
    {
        sithBot_Logf("BotLiftProbe: reached phase=%d node=%d sector=%d pos=(%.3f,%.3f,%.3f)\n",
                     sithBot_liftProbePhase, goal, sithBot_GetSectorIndex(thing->sector),
                     thing->position.x, thing->position.y, thing->position.z);
        ++sithBot_liftProbePhase;
        if (sithBot_liftProbePhase >= 3)
            return -2;
        goal = sithBot_liftProbePhase == 1 ? 144 : 4;
    }
...
static void sithBot_TickState(SithBotState *state, flex_t deltaSeconds, int deltaMs)
{
    ...
    {
        int probeGoal = sithBot_LiftProbeGoal(state, thing);
        if (probeGoal == -2)
        {
            rdVector_Zero3(&thing->physicsParams.acceleration);
            sithBot_DampHorizontalVelocity(state, thing, 14.0);
        ...
        if (probeGoal >= 0)
        {
            if (state->goalNode != probeGoal)
            {
                sithBot_Logf("BotLiftProbe: route phase=%d node=%d fromSector=%d\n",
                             sithBot_liftProbePhase, probeGoal, sithBot_GetSectorIndex(thing->sector));
                state->goalNode = probeGoal;
                state->nextNode = -1;
                state->routeGoalNode = -1;
                state->routeCommitUntilMs = 0;
            }
            ...
            state->goalMode = SITHBOT_GOAL_ROAM;
            state->nextGoalMs = sithTime_curMs + 10000;
            goto follow_route_goal;
        }
    }
    ...
    {
        int oldEnemy = state->enemyIdx;
        state->enemyIdx = sithBot_FindEnemy(state, thing, &state->enemyVisibleCached);
        if (state->enemyIdx >= 0 && state->enemyIdx != oldEnemy)
            state->reactionUntilMs = sithTime_curMs + (state->easy ? 650 : 350);
    ...
    if (enemyThing && enemyVisible && sithBot_numControlledHazards)
    {
...
{
    sithBot_liftProbePhase = 0;
    sithBot_liftProbeReadyMs = 0;
    memset(sithBot_profileUs, 0, sizeof(sithBot_profileUs));
    memset(sithBot_bots, 0, sizeof(sithBot_bots));
    sithBot_rosterReady = 0;
    sithBot_numNodes = 0;
    sithBot_navBuilt = 0;
    sithBot_navWorld = 0;
...
void sithBot_TickAll(flex_t deltaSeconds, int deltaMs)
{
    ...
    if (nowMs - sithBot_perfWindowStartMs >= 5000)
    {
        ...
        if (Main_botProfile)
        {
            sithBot_Logf("BotProfile: tickMs=%u visibilityMs=%.2f floorMs=%.2f nearestMs=%.2f pathMs=%.2f floorHits=%u\n",
                         (unsigned int)sithBot_perfTickTotalMs,
                         (double)sithBot_profileUs[0] / 1000.0,
                         (double)sithBot_profileUs[1] / 1000.0,
                         (double)sithBot_profileUs[2] / 1000.0,
                         (double)sithBot_profileUs[3] / 1000.0,
                         (unsigned int)sithBot_floorQueryHits);
            memset(sithBot_profileUs, 0, sizeof(sithBot_profileUs));
        }
        sithBot_perfWindowStartMs = nowMs;
