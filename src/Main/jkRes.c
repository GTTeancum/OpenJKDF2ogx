...
{
    { int i;
    for (i = 0; i < jkRes_gCtx.aGobDirectories[idx].numGobs; i++)
    {
...
#ifdef TARGET_XBOX
    XDBGF("jkRes_LoadGob: loading '%s' from 'episode'\n", v30);
#endif
...
int jkRes_LoadNew(jkResGobDirectory *resGob, char *name, int a3)
{
...
#ifdef TARGET_XBOX
    XDBGF("jkRes_LoadNew: name='%s' a3=%d\n", name, a3);
#endif

    jkRes_UnhookHS();
#ifdef TARGET_XBOX
    XDBG("jkRes_LoadNew: UnhookHS ok\n");
#endif
...
#ifdef TARGET_XBOX
        XDBGF("jkRes_LoadNew: NewFind mods returned %p\n", (void*)v15);
#endif
...
#ifdef TARGET_XBOX
    XDBGF("jkRes_LoadNew: NewFind('%s', 3, '%s') = %p\n", name, JKRES_GOB_EXT, (void*)v15);
#endif
    while (stdFileUtil_FindNext(v15, &v18))
    {
#ifdef TARGET_XBOX
        XDBGF("jkRes_LoadNew: found '%s'\n", v18.fpath);
#endif
        if ( resGob->numGobs >= STDGOB_MAX_GOBS )
            break;
        if ( v18.fpath[0] != '.' )
    ...
    stdFileUtil_DisposeFind(v15);
#ifdef TARGET_XBOX
    XDBGF("jkRes_LoadNew: done, numGobs=%u for '%s'\n", resGob->numGobs, name);
#endif
...
#ifdef TARGET_XBOX
    XDBGF("jkRes_NewGob: checking '%s' exists...\n", jkRes_idkGobPath);
#endif
...
int jkRes_LoadCD(int cdNumberNeeded)
{
    ...
    v24 = 0;
#ifdef TARGET_XBOX
    { xbox_debug_Print("jkRes_LoadCD: enter\n"); }
#endif
    while ( 1 )
    {
...
LABEL_39:
#ifdef TARGET_XBOX
        { xbox_debug_Print(v23 ? "jkRes_LoadCD: CD found, returning 1\n" : "jkRes_LoadCD: CD not found\n"); }
#endif
        if ( v23 )
            return 1;
    }
...
stdFile_t jkRes_FileOpen(const char *fpath, const char *mode)
{
    ...
    /* Per-file logging removed — too verbose for production.
       Uncomment for debugging file-open issues:
    XDBGF("jkRes_FileOpen: '%s' mode='%s'\n", fpath, mode); */
#endif
...
int jkRes_FilePrintf(stdFile_t fd, const char* fmt, ...)
{
    ...
    v3 = __vsnprintf(std_genBuffer, 0x400u, fmt, va);
    va_end(va);
