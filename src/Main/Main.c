...
int Main_Startup(const char *cmdline)
{
    ...
    xbox_debug_Print("Main_Startup: stdStartup...\n");
    stdStartup(&hs); // Added
    xbox_debug_Print("Main_Startup: InstallHelper_SetCwd...\n");
    InstallHelper_SetCwd(); // Added

    xbox_debug_Print("Main_Startup: wuRegistry_Startup...\n");
    wuRegistry_Startup(HKEY_LOCAL_MACHINE, "Software\\LucasArts Entertainment Company\\JediKnight\\v1.0", (BYTE*)"0.1");

    xbox_debug_Print("Main_Startup: stdHttp_Startup...\n");
    stdHttp_Startup();

    xbox_debug_Print("Main_Startup: jkGob_Startup...\n");
    jkGob_Startup();
    xbox_debug_Print("Main_Startup: jkRes_Startup...\n");
    jkRes_Startup(pHS);
    xbox_debug_Print("Main_Startup: Windows_Startup...\n");
    Windows_Startup();
    xbox_debug_Print("Main_Startup: jkStrings_Startup...\n");
    jkStrings_Startup();
    xbox_debug_Print("Main_Startup: sithCvar_Startup...\n");
    sithCvar_Startup(); // Added

    xbox_debug_Print("Main_Startup: Windows_InitWindow...\n");
    if (Windows_InitWindow())
    {
        xbox_debug_Print("Main_Startup: rdStartup...\n");
        rdStartup(&hs);
        xbox_debug_Print("Main_Startup: GUI startups...\n");
...
void Main_ParseCmdLine(char *cmdline)
{
    char* pArgTok;
    char* pArgNext;
    for (pArgTok = _strtok(cmdline, " \t"); pArgTok; pArgTok = _strtok(0, " \t"))
    {
...
int Main_FPrintf(const char* fmt, ...) {
    ...
    int ret;
    va_start (args, fmt);
    ret = __vsnprintf(std_genBuffer, 0x400, fmt, args);
    va_end (args);
