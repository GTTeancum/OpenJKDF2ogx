...
void sithRender_Draw()
{
...
#ifdef TARGET_XBOX
    { static int _srd=0; if(_srd<1){ XDBGF("sithRender_Draw: proceeding cam=%p sector=%p pCurCam=%p\n", (void*)sithCamera_currentCamera, (void*)sithCamera_currentCamera->sector, (void*)rdCamera_pCurCamera); _srd++; } }
#endif
...
#ifdef TARGET_XBOX
    { static int _srd1=0; if(_srd1<1){ XDBGF("sithRender_Draw: tint ok\n"); _srd1++; } }
#endif
...
#else
#ifdef TARGET_XBOX
        { static int _srd3=0; if(_srd3<1){ XDBGF("sithRender_Draw: Clip pFrustum=%p\n", rdCamera_pCurCamera ? (void*)rdCamera_pCurCamera->pClipFrustum : NULL); _srd3++; } }
#endif
        sithRender_Clip(sithCamera_currentCamera->sector, rdCamera_pCurCamera->pClipFrustum, 0.0, 0);
#ifdef TARGET_XBOX
        { static int _srd4=0; if(_srd4<1){ XDBGF("sithRender_Draw: Clip done\n"); _srd4++; } }
#endif
#endif
    }
    else {
    ...
    // TWL: 16ms
#ifdef TARGET_XBOX
    { static int _srn=0; if(_srn<1){ XDBGF("sithRender_Draw: numSectors=%d numSectors2=%d before RenderLevelGeo\n", sithRender_numSectors, sithRender_numSectors2); _srn++; } }
#endif
...
void sithRender_RenderLevelGeometry()
{
    ...
    for (v72 = 0; v72 < sithRender_numSectors; v72++)
    {
...
#ifdef TARGET_XBOX
        { static int _rlg=0; if(_rlg<1){ XDBGF("RLG: sector=%p numSurf=%d vec3_1=(%f,%f,%f)\n", (void*)level_idk, level_idk->numSurfaces, sithCamera_currentCamera->vec3_1.x, sithCamera_currentCamera->vec3_1.y, sithCamera_currentCamera->vec3_1.z); _rlg++; } }
#endif
        for (v75 = 0; v75 < level_idk->numSurfaces; /*v65->field_4 = sithRender_lastRenderTick,*/ ++v65, v75++)
        {
            ...
            { static int _vt=0; if(_vt<1){ XDBGF("VTx: f4=%d lastTick=%d nv=%d posIdx=%p\n", v65->field_4, sithRender_lastRenderTick, v65->surfaceInfo.face.numVertices, (void*)v65->surfaceInfo.face.vertexPosIdx); _vt++; } }
            { static int _vm=0; if(_vm<1){ XDBGF("view_matrix lvec=(%f,%f,%f) scale=(%f,%f,%f)\n", rdCamera_pCurCamera->view_matrix.lvec.x, rdCamera_pCurCamera->view_matrix.lvec.y, rdCamera_pCurCamera->view_matrix.lvec.z, rdCamera_pCurCamera->view_matrix.scale.x, rdCamera_pCurCamera->view_matrix.scale.y, rdCamera_pCurCamera->view_matrix.scale.z); _vm++; } }
            { static int _fr=0; if(_fr<1){ XDBGF("frustum zNear=%f zFar=%f farTop=%f farLeft=%f bottom=%f right=%f\n", pFullCameraFrustum->zNear, pFullCameraFrustum->zFar, pFullCameraFrustum->farTop, pFullCameraFrustum->farLeft, pFullCameraFrustum->bottom, pFullCameraFrustum->right); _fr++; } }
            ...
            if (LIKELY((sithRender_flag & 8) == 0 || v65->surfaceInfo.face.numVertices <= 3 || bIsSkySurface || !v65->surfaceInfo.face.lightingMode))
            {
...
#endif
                    { static int _cf=0; if(_cf<1){ XDBGF("ClipFace: gm=%d lm=%d nv=%d uvs=%p uvIdx=%p vpos=%p posIdx=%p\n", procEntry->geometryMode, procEntry->lightingMode, sithRender_idxInfo.numVertices, (void*)sithRender_idxInfo.vertexUVs, (void*)sithRender_idxInfo.vertexUVIdx, (void*)sithRender_idxInfo.vertices, (void*)sithRender_idxInfo.vertexPosIdx); _cf++; } }
                ...
                { static int _pr=0; if(_pr<1){ XDBGF("ProjectLst done\n"); _pr++; } }
                ...
                { static int _ap=0; if(_ap<1){ XDBGF("AddProcFace done faces=%d\n", rdCache_numProcFaces); _ap++; } }
                continue;
            }
            ...
            if (v76 > 0)
            {
                ...
                while ( 2 )
                {
                    ...
                    // MOTS added
                    if (rdGetVertexColorMode() == 0) {
                        { static int _ng=0; if(_ng<1){ XDBGF("NGon ClipFace: inten=%p v19=%d v71=%d v18=%d\n", (void*)v65->surfaceInfo.intensities, v19, v71, v18); _ng++; } }
                        ...
                        rdPrimit3_ClipFace(pSurfaceFrustum,
                                           ...
                                           &meshinfo_out,
                                           &v65->surfaceInfo.face.clipIdk);
                        { static int _ngd=0; if(_ngd<1){ XDBGF("NGon ClipFace done: nv=%d\n", meshinfo_out.numVertices); _ngd++; } }
                        /*rdPrimit3_NoClipFace(/*pSurfaceFrustum,* /
                                       v20->geometryMode,
                                       v20->lightingMode,
                                       v20->textureMode,
...
LABEL_150:
        ...
        { static int _sl=0; if(_sl<1){ XDBGF("Surface loop done, entering things loop\n"); _sl++; } }
        rdSetProcFaceUserData(level_idk->id | 0x10000);
        { static int _sp=0; if(_sp<1){ XDBGF("rdSetProcFaceUserData done, thingsList=%p\n", (void*)level_idk->thingsList); _sp++; } }
        int safeguard = 0;
        for ( i = level_idk->thingsList; i; i = i->nextThing )
        {
            ...
            { static int _tb=0; if(_tb<3){ XDBGF("ThingLoop: SphereInFrustum done v63=%d, calling RenderThing\n", v63); _tb++; } }
#ifdef SITHRENDER_SPHERE_TEST_SURFACES
            ...
            { static int _rc=0; if(_rc<3){ XDBGF("ThingLoop: calling RenderThing for real i=%p\n", (void*)i); _rc++; } }
...
//#ifndef TARGET_TWL
    ...
    rdCache_Flush();
//#endif
    { static int _rg=0; if(_rg<1){ XDBGF("RLG: rdCache_Flush done\n"); _rg++; } }
