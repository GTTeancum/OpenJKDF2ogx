...
int rdCache_SendFaceListToHardware()
{
    ...
    int tmpiter;
    int vtx_idx;       /* C89 hoist: was declared in for() at original line 699, 1069 */
    int pushed_tris;   /* C89 hoist: was declared in for() at original line 969 */
    flex_d_t intRed;        /* C89 hoist: was declared mid-block inside #ifdef JKM_LIGHTING */
    flex_d_t intGreen;
    flex_d_t intBlue;
    flex_d_t luma;          /* C89 hoist: was declared mid-block inside #ifdef SDL2_RENDER */
    flex_t z_min;           /* C89 hoist: was declared mid-block in mipmap else-branch */
    int ngt_v1, ngt_v2, ngt_v3; /* C89 hoist: was declared mid-block in NGon fan loop */
    uint8_t baseLight;      /* C89 hoist: was declared mid-block inside #ifdef JKM_LIGHTING */
    ...
    for (rend_6c_current_idx = 0; rend_6c_current_idx < rdCache_numProcFaces; rend_6c_current_idx++)
    {
        ...
        else
        {
            ...
            z_min = active_6c->z_min * rdCamera_GetMipmapScalar(); // MOTS added
        ...
        if ( v11.mipmap_related != 3 )
        {
            ...
            for (vtx_idx = 0; vtx_idx < active_6c->numVertices; vtx_idx++)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        light_level = 1.0;
                        intRed = active_6c->paRedIntensities[vtx_idx];
                        intGreen = active_6c->paGreenIntensities[vtx_idx];
                        intBlue = active_6c->paBlueIntensities[vtx_idx];

                        // Added for SDL2
#ifdef SDL2_RENDER
                        luma = (0.2126 * intRed) + (0.7152 * intGreen) + (0.0722 * intBlue);
                        light_level = luma;
            ...
            {
                ngt_v1 = active_6c->numVertices - 1;
                ngt_v2 = 1;
                ngt_v3 = 0;
                for (pushed_tris = 0; pushed_tris < active_6c->numVertices - 2; pushed_tris++)
                {
                    rdCache_aHWNormalTris[rdCache_totalNormalTris+pushed_tris].v3 = tri_vert_idx + ngt_v3;
                    rdCache_aHWNormalTris[rdCache_totalNormalTris+pushed_tris].v2 = tri_vert_idx + ngt_v2;
                    rdCache_aHWNormalTris[rdCache_totalNormalTris+pushed_tris].v1 = tri_vert_idx + ngt_v1;
...
LABEL_232:
        ...
        for (vtx_idx = 0; vtx_idx < active_6c->numVertices; vtx_idx++)
        {
            ...
            else
            {
                ...
                {
                    v91 = (rdColormap *)active_6c->colormap;
                    baseLight = *((uint8_t *)v91->lightlevel + v137->header.solidColor);
                    intRed = active_6c->paRedIntensities[vtx_idx] * 255.0 - 0.5;
                    intGreen = active_6c->paGreenIntensities[vtx_idx] * 255.0 - 0.5;
                    intBlue = active_6c->paBlueIntensities[vtx_idx] * 255.0 - 0.5;
...
int rdCache_TriCompare(const void* a_, const void* b_)
{
    ...
    /* NULL-guard: untextured faces (e.g. model solid faces) store texture=NULL.
     * Dereferencing tex_a/tex_b without this check crashes in qsort. */
    if (!tex_a && !tex_b) return 0;
    if (!tex_a) return -1;
    if (!tex_b) return  1;
