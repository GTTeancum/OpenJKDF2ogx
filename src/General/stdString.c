...
void stdString_CStrToLower(char *a1)
{
    ...
    {
#ifdef TARGET_XBOX
        /* jk.h defines __tolower as a static function pointer to a Win32
         * address (0x514550) which is garbage on Xbox.  Use inline. */
        *v1 = (result >= 'A' && result <= 'Z') ? (result + 32) : result;
#else
        *v1 = __tolower(result);
#endif
        result = v1[1];
