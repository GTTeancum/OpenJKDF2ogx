...
#ifdef TARGET_XBOX
/* Minimal mmap stubs so stdGob.c compiles — viewMapped is always 0 on Xbox */
static __inline void jk_UnmapViewOfFile(const void* p) { (void)p; }
static __inline void jk_CloseHandle(void* h) { (void)h; }
#endif
#ifndef TARGET_XBOX
/* PC-only Win32 GDI/UI and legacy DirectX wrappers.              */
/* xtl.h on Xbox provides its own versions of these APIs;         */
/* the PC stub declarations conflict with Xbox SDK types.         */
/* These functions are not called in the Xbox build path.         */
