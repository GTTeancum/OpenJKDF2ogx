...
void stdStartup(HostServices* pServices)
{
    uint16_t v1;
#ifdef TARGET_XBOX
    /* On Xbox, std_pHS is set by stdFile_xbox_Startup to point to
       g_xboxHS which has all callbacks wired. Copy the Xbox callbacks
       INTO the caller's struct so pHS also works, then point std_pHS
       at the same struct. This way jkRes_HookHS() can overwrite
       pHS->fileOpen with jkRes_FileOpen and std_pHS sees it too —
       enabling the GOB virtual filesystem for stdConffile etc. */
    if (std_pHS && pServices) {
        memcpy(pServices, std_pHS, sizeof(HostServices));
    }
    std_pHS = pServices;
#else
    std_pHS = pServices;
